...
namespace OpenCollarBot
{
    class CommandManager
    {
        ...
        public SysOut Log = null;
        public string newReply;
        public bool RunChatCommand(string cmdData, OCBotMemory ocb, GridClient client, MessageHandler.MessageHandleEvent MHE, CommandRegistry registry)
        {
            register = registry;
            ocb = OCBotMemory.Memory;
            Dictionary<UUID, int> BotAdmins = ocb.BotAdmins;
            ...
            Log.debug("Leaving command parser");
            return false;
        }

        public CommandManager(SysOut Logger, GridClient cl, OCBotMemory config, MessageHandler.MessageHandleEvent MHE)
        {
            this.cl = cl;
            mem = config;
            Log = Logger;
